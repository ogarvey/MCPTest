...
- The scene-to-resource bridge inside `0x1ae15` is now concrete enough to guide reconstruction work: the first decompressed buffer size is `field10 * field12 * 5`, and the second decompressed buffer is split with runtime pointers at `size - 0x10400` and `size - 0x10000`. The real body behind `0x14ac1` (`0x14ac9`) consumes the `0x400`-byte block as RGBX palette quads, and the xref cluster ending at `0x115ba` consumes the final `0x10000` bytes as a `256 x 256` remap / translation table. The second buffer is therefore not a tile bank, and the exact meaning of the `5` bytes per first-buffer cell is still unresolved.
...
- The second `FORGA.FAM` table does not have unique names across the full file. `Scordato` probe resolution now groups duplicate second-table names case-insensitively and uses the first matching resource when a first-table scene header points to a duplicated name.
